...
*              2. WavMusicPlay的实现是基于FreeRTOS操作系统的。
...
*********************************************************************************************************
*/
#define Wav_DMABufferSize     1024*5  /* 定义的DMA双缓冲大小，单位字节 */
...
*********************************************************************************************************
*/
static uint32_t Wav_FillAudio(FIL *_pFile, uint8_t *_ptr, uint8_t *_ptrtemp, uint16_t _usSize, uint8_t usBitsPerSample)
{
...
void WavMusicPlay(const char *filename)
{
	...
	/* 申请所需的动态内存 */
	pI2SBuffer0 = (uint8_t *)rt_alloc_mem(AppMalloc, Wav_DMABufferSize);
	pI2SBuffer1 = (uint8_t *)rt_alloc_mem(AppMalloc, Wav_DMABufferSize); 
	pI2STempBuff = (uint8_t *)rt_alloc_mem(AppMalloc, Wav_DMABufferSize/2*3);
	
	printf_Wavdbg("pI2SBuffer0 = %x, %x, %x\r\n", (int)pI2SBuffer0, (int)pI2SBuffer1, (int)pI2STempBuff);	
	while(1)
	{
		/* 等待所有任务发来事件标志 */
		...
		switch (uxBits)
		{
			...
			case MusicTaskAudioReturn_6:
				...
				rt_free_mem(AppMalloc, pI2SBuffer0);
		...
		if(uiNumBytesFill < Wav_DMABufferSize)//播放结束?
		{
			/* 等待其中一个缓冲完成 */
			uxBits = xEventGroupWaitBits(xCreatedEventGroup,    /* 事件标志组句柄 */
										 MusicTaskWaitBuffer,   /* 等待MusicTaskWaitBuffer某一位被设置 */
										 pdTRUE,                /* 退出前MusicTaskWaitBuffer位被清除，这里是任意MusicTaskWaitBuffer位被设置就“退出”*/
										 pdFALSE,               /* 设置为pdFALSE表示等待MusicTaskWaitBuffer任意位被设置*/
										 portMAX_DELAY); 	    /* 等待延迟时间 */
			...
			xEventGroupWaitBits(xCreatedEventGroup, 	  /* 事件标志组句柄 */
								MusicTaskAudioGetTime_7,  /* 等待MusicTaskAudioGetTime_7被设置 */
								pdTRUE,             	  /* 退出前MusicTaskAudioGetTime_7被清除 */
								pdFALSE,            	  /* 设置为pdTRUE表示等待MusicTaskAudioGetTime_7被设置*/
								portMAX_DELAY); 		  /* 等待延迟时间 */	
